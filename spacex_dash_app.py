...
for opt in spacex_df['Launch Site'].unique():
...
def get_pie(site):
    if site == 'All':
        figure = px.pie(spacex_df, names='Launch Site')
        return figure
    else:
        data = spacex_df[spacex_df['Launch Site'] == site]
        figure = px.pie(spacex_df, names='class')
        return figure
...
def get_scatter(site, range):
    ...
    if site == 'All':
        ...
        mask = (spacex_df['Payload Mass (kg)'] > low) & (
            spacex_df['Payload Mass (kg)'] < high) & (spacex_df['Launch Site'] == site)
        data2 = spacex_df[mask]
